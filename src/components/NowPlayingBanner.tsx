...
export default function NowPlayingBanner() {
  ...
  const [mostRecentlyPlayedGroupIndex, setMostRecentlyPlayedGroupIndex] =
    useState<number | null>(null);
  const playingGroup = getPlayingGroup(groups);
  const mostRecentlyPlayedGroup =
    mostRecentlyPlayedGroupIndex === null
      ? null
      : getGroupByIndex(mostRecentlyPlayedGroupIndex, groups) ?? null;
  const [
    isCombatToggleChecked,
    setIsCombatToggleChecked,
  ...
  const toggleIsCombatModeActive = () => {
    ...
    dispatch(
      playGroupSolo({
        groupIndex: playingGroup.index,
        isCombatModeActive: isStartingCombat,
      })
    );
  };
  ...
  useEffect(() => {
    if (playingGroup !== undefined) {
      ...
      setIsCombatToggleChecked(
        playingGroup.combatTracks.some((track) => track.isPlaying)
      );
    }
  ...
  const onPlay = () => {
    ...
    dispatch(
      playGroupSolo({
        groupIndex: mostRecentlyPlayedGroup.index,
        isCombatModeActive: isCombatToggleChecked,
      })
    );
  };
  ...
  return (
    <div className="now-playing-banner-container">
      ...
      />
      {mostRecentlyPlayedGroup !== null && (
        <PlayGroupButton
          group={mostRecentlyPlayedGroup}
          overriddenPlayFunction={onPlay}
        />
      )}
    </div>
  );
